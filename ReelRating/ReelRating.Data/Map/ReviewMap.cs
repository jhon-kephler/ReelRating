...
namespace ReelRating.Data.Map
{
    public class ReviewMap : IEntityTypeConfiguration<Review>
    {
        public void Configure(EntityTypeBuilder<Review> builder)
        {
            ...
            builder.HasIndex(x => x.StatusId).HasDatabaseName("IDX_REVIEW_STATUS");
            ...
            builder.HasOne<Categories>().WithMany().HasForeignKey(x => x.CategoriesId).HasConstraintName("FK_REVIEW_CATEGORY");
            builder.HasOne<TypeCine>().WithMany().HasForeignKey(x => x.TypeId).HasConstraintName("FK_REVIEW_TYPE");
            builder.HasOne<Status>().WithMany().HasForeignKey(x => x.StatusId).HasConstraintName("FK_REVIEW_STATUS");
        }
    }
}
